...
class LiquidityHuntModel {
    ...
    @Synchronized fun evaluate(
        ...
        val absorptionScore = (replenishment * .28 + askDepletion * .12 + stall * .22 + flowRecovery * .24 + crossStall * .14).coerceIn(0.0, 1.0)
        ...
        val earlyScore = (absorptionScore * .68 + (if (inZone) .20 else 0.0) + (if (f1 > -.12) .12 else 0.0)).coerceIn(0.0, 1.0)
        ...
        val reason: String
        when {
            below && f1 < -.25 -> { action = "ABORT — BREAKDOWN STILL ACCELERATING"; reason = "Price pushed through the sweep zone while aggressive selling remains dominant." }
            inZone && earlyScore >= .72 -> { action = "EARLY REVERSAL WINDOW"; reason = "Stop-sweep zone reached; selling is stalling while bids replenish and flow recovers." }
            inZone -> { action = "SWEEP IN PROGRESS — WATCH CLOSELY"; reason = "Price is inside the estimated liquidity pocket; exhaustion is not strong enough yet." }
            sweepScore >= .68 -> { action = "PREPARE SNIPER ENTRY"; reason = "Book structure and sell pressure point toward a probable flush into the zone below." }
            else -> { action = "WAIT — TRACKING LIQUIDITY"; reason = "No high-confidence stop-sweep entry yet." }
        }

        return Result(zoneLow, zoneHigh, sniper, abort, bounce, (sweepScore*100).roundToInt(), (absorptionScore*100).roundToInt(), action, reason)
    }
